<h1 align="center">👋 Hi, I'm Divyansh Kackker</h1>
<p align="center">
  <b>Aspiring Software Engineer · Full Stack Developer · B.E. CSE @ LNCT Bhopal</b>
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/divyansh-kackker-99b627249"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white"/></a>
  <a href="https://github.com/Divyanshkackker15"><img src="https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white"/></a>
  <a href="mailto:kackkerdivyansh1115@gmail.com"><img src="https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white"/></a>
</p>

---

### 🧑‍💻 About Me

> Pre-final year B.E. Computer Science student at LNCT Bhopal with strong foundations in **Java, DSA, and Backend Development**. I build real-world web and AI-driven applications using Spring Boot, Hibernate, MySQL, and REST APIs. Interned at **Infosys Springboard** and contributed at **AICTE IDEA Lab**. Passionate about clean architecture and scalable code.

---

### 🎓 Education

| Institution | Qualification | Year | Score |
|---|---|---|---|
| LNCT Bhopal | B.E. Computer Science | 2023 – 2027 | CGPA: 7.89 (SGPA: 8.46) |
| Bal Bhawan School | Class 12 – CBSE | 2022 – 2023 | 80.00% |
| Bal Bhawan School | Class 10 – CBSE | 2020 – 2021 | 81.90% |

---

### 💼 Experience

**Infosys Springboard — Intern** *(Sept 2025 – Nov 2025)*
> Completed Internship 6.0 on Inventory Monitoring & Reporting System — problem analysis, system design, and implementation.

**AICTE IDEA Lab — Technical Project Associate** *(Jan 2025 – Dec 2025)*
> Contributed to hands-on technical innovation projects focused on practical engineering solutions.

---

### 🚀 Projects

**📝 BlogHub**
RESTful blogging platform with secure authentication, role-based authorization, and full CRUD.
`Java` `Spring Boot` `Hibernate` `Spring Data JPA` `MySQL`

**🗳️ VoteEazy — Online Voting System**
Voting management system with Admin/Voter RBAC, candidate management, and election APIs.
`Java` `Spring Boot` `Spring Data JPA` `MySQL`

**🤖 IntelliFit — AI Posture Assessment**
AI-driven posture analysis system using camera-based pose detection for real-time pelvic alignment and clinical risk evaluation.
`AI/ML` `JavaScript` `HTML/CSS` `Pose Detection`

---

### 🛠️ Tech Stack

**Languages**
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=flat&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)

**Backend & Database**
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=spring-boot&logoColor=white)
![Hibernate](https://img.shields.io/badge/Hibernate-59666C?style=flat&logo=hibernate&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-005C84?style=flat&logo=mysql&logoColor=white)
![REST API](https://img.shields.io/badge/REST_API-FF6C37?style=flat&logo=postman&logoColor=white)

**Frontend**
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat&logo=tailwind-css&logoColor=white)

**Tools**
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![IntelliJ IDEA](https://img.shields.io/badge/IntelliJ-000000?style=flat&logo=intellij-idea&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat&logo=postman&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=flat&logo=visual-studio-code&logoColor=white)

---

### 📜 Certifications

- 📘 **Data Structures & Backend with Java** — Coursera (Board Infinity)
- 📗 **Database Management Systems** — Infosys
- ☁️ **Oracle Cloud Infrastructure — AI Foundations Associate**
- 🍃 **Spring & Spring Boot**

---

### 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Divyanshkackker15&show_icons=true&theme=default&hide_border=true&count_private=true" width="48%"/>
  <img src="https://streak-stats.demolab.com/?user=Divyanshkackker15&theme=default&hide_border=true" width="48%"/>
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Divyanshkackker15&layout=compact&theme=default&hide_border=true" width="40%"/>
</p>

---

<p align="center"><i>"First, solve the problem. Then, write the code."</i><br>
<b>Open to SWE internships & entry-level roles 🇮🇳</b></p>
